...
private:
    ...
    bool Initialize(WindowParameters window_parameters)
    {
        ...
        VulkanCookbook::InitVkDestroyer(LogicalDevice, VertexBuffer);
        if (!CreateBuffer(*LogicalDevice, sizeof(m_scene->getModel()->getMeshModel()->Data[0]) * (m_scene->getModel()->getMeshModel()->Data.size()),
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, *VertexBuffer)) {

            return false;
        }

        VulkanCookbook::InitVkDestroyer(LogicalDevice, VertexBufferMemory);
        ...
        if (!UseStagingBufferToUpdateBufferWithDeviceLocalMemoryBound(PhysicalDevice, *LogicalDevice, sizeof(m_scene->getModel()->getMeshModel()->Data[0]) * (m_scene->getModel()->getMeshModel()->Data.size()),
            &(m_scene->getModel()->getMeshModel()->Data[0]), *VertexBuffer, 0, 0, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
            GraphicsQueue.Handle, FramesResources.front().CommandBuffer, {})) {
            return false;
        }

        InitVkDestroyer(LogicalDevice, VertexBuffer2);
        if (!CreateBuffer(*LogicalDevice, sizeof(m_scene->getModel()->getMapModel()->Data[0]) * m_scene->getModel()->getMapModel()->Data.size(),
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, *VertexBuffer2)) {
            return false;
        }

        InitVkDestroyer(LogicalDevice, VertexBufferMemory2);
        if (!AllocateAndBindMemoryObjectToBuffer(PhysicalDevice, *LogicalDevice, *VertexBuffer2, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *VertexBufferMemory2)) {
            return false;
        }

        if (!UseStagingBufferToUpdateBufferWithDeviceLocalMemoryBound(PhysicalDevice, *LogicalDevice, sizeof(m_scene->getModel()->getMapModel()->Data[0]) * m_scene->getModel()->getMapModel()->Data.size(),
            &m_scene->getModel()->getMapModel()->Data[0], *VertexBuffer2, 0, 0, VK_ACCESS_VERTEX_ATTRIBUTE_READ_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
            GraphicsQueue.Handle, FramesResources.front().CommandBuffer, {})) {
            return false;
        }
        ...
        *Pipeline[1] = pipeline[0];
        ...
        InitVkDestroyer(LogicalDevice, DescriptorSetLayout2);
        ...
        InitVkDestroyer(LogicalDevice, DescriptorPool2);
        if (!CreateDescriptorPool(*LogicalDevice, false, 1, { descriptor_pool_size2 }, *DescriptorPool2)) {
            return false;
        }

        if (!AllocateDescriptorSets(*LogicalDevice, *DescriptorPool2, { *DescriptorSetLayout2 }, DescriptorSets2)) {
            return false;
        }

        BufferDescriptorInfo buffer_descriptor_update2 = {
          DescriptorSets2[0],                          // VkDescriptorSet                      TargetDescriptorSet
          0,                                          // uint32_t                             TargetDescriptorBinding
          0,                                          // uint32_t                             TargetArrayElement
          VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,          // VkDescriptorType                     TargetDescriptorType
          {                                           // std::vector<VkDescriptorBufferInfo>  BufferInfos
        ...
        UpdateDescriptorSets(*LogicalDevice, {}, { buffer_descriptor_update2 }, {}, {});

        // Graphics pipeline

        InitVkDestroyer(LogicalDevice, PipelineLayout2);
        if (!CreatePipelineLayout(*LogicalDevice, { *DescriptorSetLayout2 }, {}, *PipelineLayout2)) {
            return false;
        }

        std::vector<unsigned char> vertex_shader_spirv2;
        if (!GetBinaryFileContents("Data/Shaders/11 Lighting/01 Rendering a geometry with vertex diffuse lighting/shader.vert.spv", vertex_shader_spirv2)) {
            return false;
        }

        VkDestroyer(VkShaderModule) vertex_shader_module2;
        InitVkDestroyer(LogicalDevice, vertex_shader_module2);
        if (!CreateShaderModule(*LogicalDevice, vertex_shader_spirv2, *vertex_shader_module2)) {
            return false;
        }

        std::vector<unsigned char> fragment_shader_spirv2;
        if (!GetBinaryFileContents("Data/Shaders/11 Lighting/01 Rendering a geometry with vertex diffuse lighting/shader.frag.spv", fragment_shader_spirv2)) {
            return false;
        }
        VkDestroyer(VkShaderModule) fragment_shader_module2;
        InitVkDestroyer(LogicalDevice, fragment_shader_module2);
        if (!CreateShaderModule(*LogicalDevice, fragment_shader_spirv2, *fragment_shader_module2)) {
            return false;
        }

        std::vector<ShaderStageParameters> shader_stage_params2 = {
          {
            VK_SHADER_STAGE_VERTEX_BIT,       // VkShaderStageFlagBits        ShaderStage
            *vertex_shader_module2,            // VkShaderModule               ShaderModule
            "main",                           // char const                 * EntryPointName
            nullptr                           // VkSpecializationInfo const * SpecializationInfo
          },
          {
            VK_SHADER_STAGE_FRAGMENT_BIT,     // VkShaderStageFlagBits        ShaderStage
            *fragment_shader_module2,          // VkShaderModule               ShaderModule
            "main",                           // char const                 * EntryPointName
            nullptr                           // VkSpecializationInfo const * SpecializationInfo
          }
        };

        std::vector<VkPipelineShaderStageCreateInfo> shader_stage_create_infos2;
        SpecifyPipelineShaderStages(shader_stage_params2, shader_stage_create_infos2);

        std::vector<VkVertexInputBindingDescription> vertex_input_binding_descriptions2 = {
          {
            0,                            // uint32_t                     binding
        ...
        std::vector<VkVertexInputAttributeDescription> vertex_attribute_descriptions2 = {
          {
            0,                                                                        // uint32_t   location
            0,                                                                        // uint32_t   binding
            VK_FORMAT_R32G32B32_SFLOAT,                                               // VkFormat   format
        ...
        VkPipelineVertexInputStateCreateInfo vertex_input_state_create_info2;
        SpecifyPipelineVertexInputState(vertex_input_binding_descriptions2, vertex_attribute_descriptions2, vertex_input_state_create_info2);

        VkPipelineInputAssemblyStateCreateInfo input_assembly_state_create_info2;
        SpecifyPipelineInputAssemblyState(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, false, input_assembly_state_create_info2);

        VkGraphicsPipelineCreateInfo pipeline_create_info2;
        SpecifyGraphicsPipelineCreationParameters(0, shader_stage_create_infos2, vertex_input_state_create_info2, input_assembly_state_create_info2,
            nullptr, &viewport_state_create_info, solid_rasterization_state_create_info, &multisample_state_create_info, &depth_stencil_state_create_info, &blend_state_create_info,
            &dynamic_state_create_info, *PipelineLayout2, *RenderPass, 0, VK_NULL_HANDLE, -1, pipeline_create_info2);

        std::vector<VkPipeline> pipeline2;
        if (!CreateGraphicsPipelines(*LogicalDevice, { pipeline_create_info2 }, VK_NULL_HANDLE, pipeline2)) {
            return false;
        }

        InitVkDestroyer(LogicalDevice, Pipeline2);
        *Pipeline2 = pipeline2[0];
    ...
    bool Draw()
    {
        auto prepare_frame = [&](VkCommandBuffer command_buffer, uint32_t swapchain_image_index, VkFramebuffer framebuffer) {
            ...
            BindVertexBuffers(command_buffer, 0, { { *VertexBuffer2, 0 } });
            ...
            for (size_t i = 0; i < m_scene->getModel()->getMapModel()->Parts.size(); ++i) {
                DrawGeometry(command_buffer, m_scene->getModel()->getMapModel()->Parts[i].VertexCount, 1, m_scene->getModel()->getMapModel()->Parts[i].VertexOffset, 0);
            }
            ...
            BindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *PipelineLayout2, 0, DescriptorSets2, {});

            BindPipelineObject(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *Pipeline2);

            for (size_t i = 0; i < m_scene->getModel()->getMeshModel()->Parts.size(); ++i) {
                DrawGeometry(command_buffer, m_scene->getModel()->getMeshModel()->Parts[i].VertexCount, 1, m_scene->getModel()->getMeshModel()->Parts[i].VertexOffset, 0);
            }
    ...
    bool UpdateStagingBuffer(bool force)
    {
        ...
        if (MouseState.Buttons[0].IsPressed || force) {

            if (MouseState.Buttons[0].IsPressed) {

                m_scene->getCamera()->RotateHorizontally(0.5f * MouseState.Position.Delta.X);
                m_scene->getCamera()->RotateVertically(-0.5f * MouseState.Position.Delta.Y);
            ...
            if (MouseState.Wheel.WasMoved) {
                ...
                m_scene->getCamera()->ChangeDistance(MouseState.Wheel.Distance);
                force = true;
            }

            Matrix4x4 translation_matrix = PrepareTranslationMatrix(0.0f, 0.0f, 0.0f);
            Matrix4x4 model_view_matrix = m_scene->getCamera()->GetMatrix() * translation_matrix;
